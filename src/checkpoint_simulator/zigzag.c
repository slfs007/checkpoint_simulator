...
void db_zigzag_ckp( int ckp_order,void *zigzag_info)
{
    ...
    for (i = 0;i < db_size; i ++){
        if (0 == info->db_zigzag_mw[i]){
            fwrite( (void *)(info->db_zigzag_as1 + i * DBServer.unitSize),DBServer.unitSize,1,ckp_file);
        }else{
            fwrite( (void *)(info->db_zigzag_as0 + i * DBServer.unitSize),DBServer.unitSize,1,ckp_file);
        }
    }
