    ...
    if (0 == info->current){
        for (i = 0; i < db_size; i ++){
            if (1 == info->db_pp_as_even[i]){
                ...
                memcpy(info->db_pp_as_previous + i * DBServer.unitSize,
                        info->db_pp_as_even + i * DBServer.unitSize,DBServer.unitSize);
                memset(info->db_pp_as_even + i * DBServer.unitSize,0,DBServer.unitSize);
        ...
        for (i = 0; i < db_size; i ++){
            if (1 == info->db_pp_as_odd[i]){
                memcpy(info->db_pp_as_previous + i * DBServer.unitSize,
                        info->db_pp_as_odd + i * DBServer.unitSize,DBServer.unitSize);
                memset(info->db_pp_as_odd + i * DBServer.unitSize,0,DBServer.unitSize);
                info->db_pp_odd_ba[i] = 0;
            }
        }
    }
    //printf("WRITE:%d\n",DBServer.unitSize * db_size);
